...
/**
 * The EntityBarcodeTracker class is responsible for initializing and managing the barcode detection
 * and tracking process using a BarcodeDecoder and EntityTrackerAnalyzer. This class is designed to
 * work within an Android application context and facilitates asynchronous operations for barcode
 * tracking in a view-referenced coordinate system.

 * The EntityBarcodeTracker configures the decoder, assigns an analyzer for image analysis, and provides
 * methods to stop and dispose of resources when they are no longer needed. It also provides a callback
 * mechanism to notify when the tracker is ready and to handle detection results.

 * Usage:
 * - Instantiate the EntityBarcodeTracker with the necessary context, callback, and image analysis configuration.
 * - The class automatically initializes the BarcodeDecoder and EntityTrackerAnalyzer.
 * - Implement the DetectionCallback interface to handle detection results and tracker readiness.
...
class EntityBarcodeTracker(
    ...
    interface DetectionCallback {
        fun handleEntitiesForEntityView(result: EntityTrackerAnalyzer.Result)

        // Method to notify when the tracker is ready for use.
        fun onEntityBarcodeTrackerReady() {}
    }
     ...
     */
    fun getEntityTrackerAnalyzer() = entityTrackerAnalyzer
